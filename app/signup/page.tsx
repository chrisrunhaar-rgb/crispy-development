        ...
        <div style={{ marginBottom: "2rem" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
            <PathwayOption
              ...
              description="Individual access — grow as a cross-cultural leader at your own pace."
            />
            <PathwayOption
              value="team"
              title="Team Pathway"
              description="For team leaders — manage members, select content, and grow together."
            />
          </div>
        </div>
...
function PathwayOption({ value, title, description }: { value: string; title: string; description: string }) {
  return (
    <label className="pathway-option" style={{ cursor: "pointer" }}>
      <input
        type="radio"
        name="pathway"
        value={value}
        style={{ marginTop: "0.125rem", accentColor: "oklch(30% 0.12 260)", flexShrink: 0 }}
        defaultChecked={value === "personal"}
      />
      <div>
        <p style={{ fontFamily: "var(--font-montserrat)", fontWeight: 700, fontSize: "0.9375rem", color: "oklch(22% 0.005 260)", marginBottom: "0.25rem" }}>{title}</p>
        <p style={{ fontFamily: "var(--font-montserrat)", fontSize: "0.8125rem", color: "oklch(52% 0.008 260)", lineHeight: 1.5 }}>{description}</p>
      </div>
    </label>
  );
}
