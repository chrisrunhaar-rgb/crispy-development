...
export async function saveFiveLanguagesTeamResult(
  ...
  const { error } = await supabase.from("team_member_results").upsert(
    {
      team_id: teamId,
      user_id: user.id,
      result_type: "5languages",
      result_key: `${receivingPrimary}|${givingPrimary}`,
      scores: { receiving: receivingScores, giving: givingScores },
