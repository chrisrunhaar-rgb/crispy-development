...
export default function PeerGroupsContent({ groups, ctaHref = "/membership", isMember = false }: { groups: Group[]; ctaHref?: string; isMember?: boolean }) {
  ...
  return (
    <>
      ...
      {/* ── WORLD MAP ── */}
      <section style={{ background: "oklch(97% 0.005 80)", paddingBlock: "clamp(4rem, 6vw, 6rem)", borderTop: "1px solid oklch(88% 0.008 80)" }}>
        <div className="container-wide">
          <p className="t-label" style={{ color: "oklch(65% 0.15 45)", marginBottom: "0.875rem", fontSize: "0.62rem" }}>{p.mapLabel}</p>
          <h2 className="t-section" style={{ color: "oklch(22% 0.10 260)", marginBottom: "0.75rem" }}>{p.mapHeading}</h2>
          <p style={{ fontFamily: "var(--font-montserrat)", fontSize: "0.9375rem", color: "oklch(42% 0.008 260)", marginBottom: "2rem", maxWidth: "44ch" }}>
            {p.mapBody}
          </p>
